...
factory = {
    clusterMovies: function(useFeatureSelection, featureCount, featureMethod, maxClusterCount, labelCount) {
        return fs.readdirAsync("./input/").then(fileNames => {
            ...
            _.forEach(fileNames, (movideDir, index) => {
                if (ignoreList.indexOf(movideDir) == -1) {
            ...
            for(var i = 2; i < maxClusterCount; i++) {
                ...
                clusters = hac.getClustersWithLabels(i, ["id", "content"], labelCount, featureMethod);
                var result = "";
